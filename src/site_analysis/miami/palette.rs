//! Qualitative color palettes for Miami cell-type panels.
//!
//! Lifted from legume-rs `plot-utils` so `faba` does not depend on that
//! crate. Default picks favor colorblind-safe tables below 8 and fall
//! back to Tableau Category10/20 for larger panels.
//!
//! ## References
...
/// Resolve `Auto` to a concrete palette based on `n` panels.
#[must_use]
